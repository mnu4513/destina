...
const Section = () => {
    ...
    // Edit source handler
    async function handleEdit(e, id, name, slug) {
        e.preventDefault();
        setEditSourceId(id);
        setNameEdit(name);
        setSlugEdit(slug);
    ...
    async function handleSaveEdit(e) {
        e.preventDefault();
        try {
            const response = await fetch(baseUrl + '/admin/section/' + editSourceId, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function handleDelete(e, id) {
        e.preventDefault();
        try {
            const response = await fetch(baseUrl + '/admin/section/' + id, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
                                ...
                                {filterList.map((source) => (
                                    <tr key={source.id} className="bg-white border-b text-gray-600">
                                        ...
                                        <th scope="row" className="px-6 py-3 text-right">
                                            <button className='text-sm  rounded-md py-1 px-2' onClick={(e) => handleEdit(e, source.id, source.name, source.slug)}> <img src={edit_icon} alt='edit' className='h-5' /> </button>
