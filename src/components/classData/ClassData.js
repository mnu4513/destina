...
const ClassData = () => {
    ...
    async function handleCreate(e) {
        e.preventDefault();
        const sectionAr = section.split(',');
        try {
            const response = await fetch(baseUrl + '/admin/class', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ name, slug, section: sectionAr })
            });
    ...
    async function toggleStatus(e, id) {
        e.preventDefault();
        try {
            const response = await fetch(`${baseUrl}/admin/class/${id}/status`, {
                method: 'PUT',
                headers: {
    ...
    // Edit source handler
    async function handleEdit(e, id, name, slug, section) {
        e.preventDefault();
        setEditSourceId(id);
        setNameEdit(name);
        setSlugEdit(slug);
        const sectionStr = section?.map((x) => x.id).join(',');
        setSectionEdit(sectionStr);
    ...
    async function handleSaveEdit(e) {
        e.preventDefault();
        const sectionAr = sectionEdit.split(',').map(Number);
        console.log(sectionAr);
        try {
            const response = await fetch(baseUrl + '/admin/class/' + editSourceId, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ name: nameEdit, slug: slugEdit, section: sectionAr })
            });
    ...
    async function handleDelete(e, id) {
        e.preventDefault();
        try {
            const response = await fetch(baseUrl + '/admin/class/' + id, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    return (
        <div className='flex flex-col lg:flex-row w-full'>
            ...
            <div className='border-2 flex-grow min-w-min max-h-64 border-gray-200 px-5 py-2 space-y-3 mt-4 mx-auto w-full'>
                ...
                <input type='text' value={section} placeholder={`Use ',' to enter multiple sections`} onChange={(e) => setSection(e.target.value)} className='border-2 border-gray-500 px-2 py-1 my-2 rounded-md w-full' />
                <hr />
                                ...
                                {filterList.map((source) => (
                                    <tr key={source.id} className="bg-white border-b text-gray-600">
                                        ...
                                        <th scope="row" className="px-6 py-3 text-right">
                                            <button className='text-sm  rounded-md py-1 px-2' onClick={(e) => handleEdit(e, source.id, source.name, source.slug, source.section)}> <img src={edit_icon} alt='edit' className='h-5' /> </button>
                    ...
                    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-50">
                        <div className="bg-white p-8 rounded-md">
                            ...
                            <input
                                type="text"
                                value={sectionEdit}
                                placeholder={`Use ',' to enter multple section`}
